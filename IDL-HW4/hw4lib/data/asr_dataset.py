...
class ASRDataset(Dataset):
        ...
        for i in tqdm(range(self.length)):
            ...
            if self.partition != "test-clean":
                ...
                self.transcripts_shifted.append(
                    torch.tensor([self.sos_token] + tokenized, dtype=torch.long)
                )
                self.transcripts_golden.append(
                    torch.tensor(tokenized + [self.eos_token], dtype=torch.long)
                )
    ...
    def collate_fn(self, batch) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        batch_feats  = [item[0] for item in batch]  # Extract features from (feat, shifted, golden) tuples
        ...
        feat_lengths = [feat.shape[1] for feat in batch_feats] # B

        # TODO: Pad features to create a batch of fixed-length padded features
        # Note: Use torch.nn.utils.rnn.pad_sequence to pad the features (use 0.0 as the padding value for features)
        # Features need to be transposed to (time, num_feats) for pad_sequence, then transposed back
        batch_feats_transposed = [feat.transpose(0, 1) for feat in batch_feats]  # (time, num_feats)
        padded_feats = pad_sequence(batch_feats_transposed, batch_first=True, padding_value=0.0) # B x T x F
        ...
        if self.partition != "test-clean":
            ...
            transcript_lengths = [len(transcript) for transcript in batch_shifted] # B  
        ...
        transcript_lengths_tensor = torch.tensor(transcript_lengths) if transcript_lengths is not None else None
        return padded_feats, padded_shifted, padded_golden, torch.tensor(feat_lengths), transcript_lengths_tensor
