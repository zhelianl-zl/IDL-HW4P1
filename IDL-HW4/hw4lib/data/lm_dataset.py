...
class LMDataset(Dataset):
    ...
    def __init__(
        ...
        self.text_files = sorted(files)

        # TODO: Take subset
        subset_frac = self.config.get("subset", 1.0)  # 默认用全部
        if subset_frac >= 1.0:
            subset_size = len(self.text_files)
        else:
            subset_size = max(1, int(len(self.text_files) * subset_frac))
